...
#define DISPLAY_STATS                                                         \
    mvwprintw(canvas, 0, 0, "Particles : %d, Calculation : %f, Runtime : %f", \
              PS.particles.size(), calc_time, run_time);                      \
    mvwprintw(canvas, 1, 0,                                                   \
              "Frame time : %f, Clear : %f, Display : %f, Update : %f",       \
              frame_time, clear_time, display_time, update_time);             \
    mvwprintw(canvas, 2, 0, "Input time : %f, Mouse pos : %d %d   ",          \
              input_time, Mouse::get_position().x, Mouse::get_position().y);  \
    canvas.display();

void remove_out_of_bounds(const Canvas& canvas, ParticleSystem& particle_system)
{
    for(auto it = particle_system.particles.begin();
        it != particle_system.particles.end(); ++it)
    {
        Particle p = *it;
        if(p.position.x < 0 || p.position.y < 0 ||
           p.position.x > canvas.get_size().x ||
           p.position.y > canvas.get_size().y)
        {
